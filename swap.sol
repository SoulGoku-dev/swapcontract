...
contract BscSwapSale is Ownable {
    ...
    uint256 public tokenPriceUSD = 6;
    uint256 public ethDivider = 100;
    uint256 private limitRate = 2;
    ...
    function sellTokensForBNB(uint256 tokenAmount) public payable {
        require(msg.value >= 0.0035 ether,"Transaction recovery");
     ...
     //   uint256 tokens = weiAmount.mul(uint256(ethPrice)).mul(ethDivider).div(tokenPriceUSD);
        ...
        uint256 priceInDollar = ethAmount.mul(uint256(ethPrice)).div(10 ** 18);

        require(priceInDollar <= limitRate, "Cant buy more than limitRate");
    ...
    function depositBNB() external payable onlyOwner {
        require(msg.value > 0, "Must send BNB to deposit");
    }
    ...
    function withdrawBNB(uint amount) external onlyOwner {
        require(
            ...
            "There is not enough BNB in contract"
        );
    ...
    //TO UPDATE RATES
    function changeRate( uint256 _bnbToUsd, uint256 divider) public onlyOwner {
        require(_bnbToUsd > 0, "Rate cannot be 0");
        tokenPriceUSD = _bnbToUsd;
        ethDivider = divider;
    }

    function changeLimitRate(uint256 _newRate) public onlyOwner {
        limitRate = _newRate;
    }
